...
export default function Word2PDF() {
  ...
  const handleFileUpload = async () => {8
    ...
    if (file) {
      ...
      try {
        const response: AxiosResponse = await axios.post(`${BASE_URL}/upload_word`, formData, {
          headers: {
  ...
  const handleFileDownload = async () => {
    try {
      const response: AxiosResponse<Blob> = await axios.get(`${BASE_URL}/download_word`, {
  ...
  return (
    <>
      <Header text="Convert Word to PDF Document"/>
      <input
